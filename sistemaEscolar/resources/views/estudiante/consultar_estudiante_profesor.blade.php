    ...
    <table class="table tabla-consultar">
        ...
        <tbody>
            ...
            <tr class="tabla-consultar">
                ...
                <td><button type="button" id="modificarEstudiante" class="btn-opcion"><a class="texto-link" href="{{ route('estudianteProfesorContraseña', $estudiante->id) }}">Cambiar Contraseña ✍️</a></button></td>
                <td><button type="button" id="modificarEstudiante" class="btn-opcion"><a class="texto-link" href="{{ route('estudianteProfShow', $estudiante->id) }}">Modificar ✍️</a></button></td>
                <td>
                    <form method="post" action="{{ route('estudianteProfDelete', $estudiante->id) }}">
                        @method('DELETE')
                        @csrf
                        <button type="submit" id="eliminarEstudiante" class="btn-opcion btn-opcion-color"><a class="texto-link">Eliminar 🗑️</a></button>
                    </form>
                </td>
            </tr>
            @endforeach
